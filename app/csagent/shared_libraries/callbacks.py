...
from csagent.entities.customer import Customer
...
def validate_customer_id(customer_id: str, session_state: State) -> Tuple[bool, str]:
    ...
    try:
        ...
        if customer_id == c.uuid:
            return True, None
        else:
            return False, "You cannot use the tool with customer_id " +customer_id+", only for "+c.uuid+"."
...
def before_agent(callback_context: InvocationContext):
    ...
    if "customer_profile" not in callback_context.state:
        callback_context.state["customer_profile"] = Customer.get_customer().to_json()
