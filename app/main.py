...
async def start_agent_session(user_id, is_audio=False):
    ...
    # Set response modality (default to AUDIO)
...
async def sse_endpoint(user_id: int, is_audio: str = "false"):
    ...
    active_sessions[user_id_str] = SessionState(live_request_queue)

    print(f"Client #{user_id} connected via SSE, audio mode: {is_audio}")

    def cleanup():
        # Close queue and clear state
        live_request_queue.close()
        state = active_sessions.get(user_id_str)
        if state:
...
async def send_message_endpoint(user_id: int, request: Request):
    ...
    state: SessionState | None = active_sessions.get(user_id_str)
    if not state:
    ...
    if mime_type == "text/plain":
        ...
        # Send first chunk with activity_start to open an input segment
        ...
        # Debounce end-of-utterance: if no audio arrives for ~350ms, signal activity_end
        state.cancel_silence_task()
        ...
        # Accept screen-share frames (e.g., image/jpeg or image/png)
        now = asyncio.get_event_loop().time()
        # Drop frames faster than 1.5s to avoid backpressure
        if state.last_image_ts is None or (now - state.last_image_ts) >= 1.5:
            ...
            # Silently drop to keep audio responsive
            pass
    else:
