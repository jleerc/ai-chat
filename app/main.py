...
async def start_agent_session(user_id, is_audio=False):
    ...
    # Set response modality based on client capability
...
async def send_message_endpoint(user_id: int, request: Request):
    ...
    if mime_type == "text/plain":
        ...
        async def end_after_silence(current_mime=mime_type):
            try:
        ...
        # Accept frames at ~1s cadence (aligned with client)
        if state.last_image_ts is None or (now - state.last_image_ts) >= 0.9:
            decoded_data = base64.b64decode(data)
            state.queue.send_realtime(Blob(data=decoded_data, mime_type=mime_type))
